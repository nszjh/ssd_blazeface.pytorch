...
class Detect(Function):
    ...
    def forward(self, all_data, conf_data, prior_data):
        ...
        output1 = torch.zeros(num, self.num_classes, self.top_k, 15)
        ...
        for i in range(num):
            ...
            for cl in range(1, self.num_classes):
                ...
                output1[i, cl, :count] = torch.cat((output[i, cl, :count], lanmarks[ids[:count]]), 1)

        # flt = output.contiguous().view(num, -1, 5)
        flt = output1.contiguous().view(num, -1, 15)

        _, idx = flt[:, :, 0].sort(1, descending=True)
        _, rank = idx.sort(1)
        flt[(rank < self.top_k).unsqueeze(-1).expand_as(flt)].fill_(0)
        print (flt.shape, output1.shape)
        return output1
