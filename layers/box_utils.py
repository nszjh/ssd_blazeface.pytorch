...
def match1(threshold, truths, priors, variances, labels, landmark, loc_t, conf_t, landmark_t, idx):
    ...
    # print ("landmark: ", matches_land, land)
...
def encode_landmark(matched, priors, variances):
    ...
    # 
    g_xy1 = (matched[:, :2] - priors[:, :2]) / variances[0]
    g_xy2 = (matched[:, 2:4] - priors[:, :2]) / variances[0]
    g_xy3 = (matched[:, 4:6] - priors[:, :2]) / variances[0]
    g_xy4 = (matched[:, 6:8] - priors[:, :2]) / variances[0]
    g_xy5 = (matched[:, 8:10] - priors[:, :2]) / variances[0]
    # encode variance


    # return target for smooth_l1_loss
    return torch.cat([g_xy1, g_xy2, g_xy3, g_xy4, g_xy5], 1)  # [num_priors,10]
    # return matched / variances[0]
...
def decode_landmark(land, priors, variances):
    ...
    land1 = torch.cat([land[:, :2] * variances[0] + priors[:, :2], land[:, 2:4] * variances[0] + priors[:, :2], \
    land[:, 4:6] * variances[0] + priors[:, :2], land[:, 6:8] * variances[0] + priors[:, :2], land[:, 8:10] * variances[0] + priors[:, :2]], 1)  # [num_priors,10]
    # encode variance
    # return target for smooth_l1_loss
